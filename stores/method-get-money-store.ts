...
export const useMethodGetMoneyStore = defineStore('method_get_money', () => {
  ...
  const table = ref<string>('method_get_money') // Получение наименования категории(таблицы)
  ...
  */
  async function deleteRecord(data: any) {
    ...
    const paramsQuery: Query = { url: `/api/method_get_money/delete`, method: 'delete', body: data } // параметры запроса
    try {
      let response: any = await query(paramsQuery) // Отправка запроса на удаление
      await getList()
      return response.data
