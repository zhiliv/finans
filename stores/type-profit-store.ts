...
export const useStore = defineStore('type-profit', () => {
  ...
  const table = ref<string>('type_profit') // Получение наименования категории(таблицы)
  ...
  */
  async function deleteRecord(data: any) {
    ...
    const paramsQuery: Query = { url: `/api/type_profit/delete`, method: 'delete', body: data } // параметры запроса
    try {
      let response: any = await query(paramsQuery) // Отправка запроса на удаление
