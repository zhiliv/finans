...
export default defineEventHandler(async (event: H3Event) => {
  ...
  try {
    ...
    if (count === 0) {
      ...
      const linkCount = await sequelize.models.link_type_docs.count({ where: { id_type_doc: params.id } }) // Получение количества связанных записей
      if (linkCount > 0) {
        response.status = 202 // установка статуса ответа
        response.typeMessage = 'warning' // установка типа ответа
      ...
      response.data = await sequelize.models.type_docs.destroy(optionsWhere) // удаление данных
    }
    return response
  } catch (error: any) {
